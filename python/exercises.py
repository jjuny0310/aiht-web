...
def squat_run(pose_landmarks):
    ...
    # 신뢰도 체크(모든 관절이 정확히 나오면 자세교정 사운드 출력 가능)
    ...
    if squat_state == 0:
        ...
        # 발 각도 체크
        if foot_angle_range[0] <= left_foot_angle <= foot_angle_range[1] and keypoints[LEFT_FOOT_INDEX][0] > \
                keypoints[LEFT_HEEL][0] \
                and foot_angle_range[0] <= right_foot_angle <= foot_angle_range[1] and keypoints[RIGHT_FOOT_INDEX][
            0] < keypoints[RIGHT_HEEL][0]:
            foot_state = "pass"

        else:
            if foot_angle_range[0] > left_foot_angle or foot_angle_range[0] > right_foot_angle or \
                    keypoints[LEFT_FOOT_INDEX][0] <= keypoints[LEFT_HEEL][0] or \
...
def pushup_run(pose_landmarks):
    ...
    if keypoints[LEFT_SHOULDER][0] > keypoints[NOSE][0] or keypoints[RIGHT_SHOULDER][0] > \
            keypoints[NOSE][0]:
        # 신뢰도 체크(모든 관절이 정확히 나오면 자세 교정 사운드 출력 가능)
        ...
        if np.argmax(predict[0]) == 0:
            ...
            # 엉덩이 ~ 어깨 거리 체크
            if hip_distance_range[0] <= shoulder_to_hip <= hip_distance_range[1] and keypoints[LEFT_SHOULDER][1] <= \
    ...
    # 푸쉬업(R)
        ...
        if np.argmax(predict[0]) == 0:
            ...
            # 엉덩이 ~ 어깨 거리 체크
            if hip_distance_range[0] <= shoulder_to_hip <= hip_distance_range[1] and keypoints[RIGHT_SHOULDER][1] <= \
